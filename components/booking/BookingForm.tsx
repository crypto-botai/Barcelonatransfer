...
import { MapPin, Calendar, Clock, Users, Zap, ArrowRight, Loader2, ChevronDown } from "lucide-react";
...
export default function BookingForm({ compact = false }: Props) {
  ...
  const initAutocomplete = useCallback((
    ...
    setter: (v: { address: string; lat: number; lng: number }) => void
  ) => {
  ...
  useEffect(() => {
    if (!mapsReady) return;
    if (pickupRef.current)  initAutocomplete(pickupRef.current,  setPickup);
    if (dropoffRef.current) initAutocomplete(dropoffRef.current, setDropoff);
    ...
    <div className={cn(
      ...
      <div className="space-y-3">
        ...
        <div className="relative">
          <select
            value={vehicle}
            onChange={(e) => setVehicle(e.target.value as VehicleClass)}
              ...
              <div className="bg-gold-500/5 border border-gold-500/20 rounded-xl p-4">
                ...
                <div className="grid grid-cols-3 gap-3 text-center">
                  <div>
                    ...
                    <p className="text-white text-sm font-medium">{quote.distanceKm} km</p>
                  </div>
                  <div>
                    <p className="text-dark-400 text-xs">Duration</p>
                    <p className="text-white text-sm font-medium">{quote.durationMin} min</p>
                  </div>
                  <div>
                    <p className="text-dark-400 text-xs">All-inclusive</p>
                    <p className="text-green-400 text-sm font-medium">Fixed</p>
                  </div>
                </div>
              </div>
            </motion.div>
        ...
        <button
          onClick={handleBook}
          disabled={!pickup.address || !dropoff.address || !date || !time}

  ...
  return (
    <main className="min-h-screen bg-[#050505] flex items-center justify-center p-4 relative overflow-hidden">
      ...
      >
        ...
        <p className="text-dark-400 text-sm mb-7">Sign in to manage your bookings</p>
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input required type="email" value={email} onChange={(e) => setEmail(e.target.value)}
                placeholder="your@email.com" className="input-luxury w-full pl-10 pr-4 py-4 rounded-xl" />
            </div>
          </div>
          <div>
            <label className="text-xs text-dark-400 uppercase tracking-wider block mb-1.5">Password</label>
            <div className="relative">
              <Lock size={14} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-gold-500/60" />
              <input required type={show ? "text" : "password"} value={password} onChange={(e) => setPassword(e.target.value)}
                placeholder="••••••••" className="input-luxury w-full pl-10 pr-10 py-4 rounded-xl" />
        ...
        <div className="relative my-6">
          ...
          <div className="relative flex justify-center text-xs text-dark-500">
            <span className="px-3 bg-[#111]">or</span>
          </div>
        </div>

        <button onClick={() => signIn("google", { callbackUrl: "/dashboard" })}
          className="btn-outline-gold w-full py-3.5 rounded-xl font-semibold flex items-center justify-center gap-3 text-sm">
